...
export const salesOrders = createCrudPage({
  ...
  columns: [
    { key: 'urgent_yn', label: '긴급', align: 'center', render: (r) => r.urgent_yn ? `<span class="badge badge--danger">긴급</span>` : '', csv: (r) => (r.urgent_yn ? '긴급' : '') },
    { key: 'order_no', label: '수주번호', cls: 'cell-code', sortable: true },
    { key: 'order_date', label: '수주일', type: 'date', sortable: true },
    { key: 'partner', label: '거래처(고객사)' },
    { key: 'ship_to', label: '납품처', render: (r) => escapeHtml(r.ship_to || r.partner || ''), csv: (r) => (r.ship_to || r.partner || '') },
  ...
  fields: [
    ...
    { key: 'urgent_yn', label: '긴급 발주', type: 'switch', default: false },
    { key: 'partner', label: '거래처(고객사)', required: true, ref: { table: 'partners', value: 'name', label: (r) => `${r.code} · ${r.name}` }, placeholder: '고객사 선택' },
    { key: 'ship_to', label: '납품처', ref: { table: 'partners', value: 'name', label: (r) => `${r.code} · ${r.name}` }, placeholder: '고객사와 다를 경우 지정 (분납 시 납품처 선택)' },
...
export const shippingOrders = createCrudPage({
  ...
  fields: [
    ...
    { key: 'order_no', label: '수주번호', ref: { table: 'sales_orders', value: 'order_no', label: (r) => `${r.urgent_yn ? '[긴급] ' : ''}${r.order_no} · ${r.item_name} (${r.partner || ''})`, fill: { partner: 'partner', ship_to: 'ship_to', urgent_yn: 'urgent_yn', item_code: 'item_code', item_name: 'item_name', ship_qty: 'order_qty' } }, placeholder: '수주 선택' },
    { key: 'partner', label: '거래처(고객사·자동)', readonly: true },
    { key: 'ship_to', label: '납품처', ref: { table: 'partners', value: 'name', label: (r) => `${r.code} · ${r.name}` }, placeholder: '수주의 납품처 자동, 변경 가능' },
    { key: 'urgent_yn', label: '긴급 발주', type: 'switch', default: false },
