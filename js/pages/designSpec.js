...
const SCREENS = [
  ...
  S({ id: 'item', title: '품목관리', group: '기준정보관리', path: '/base/items', type: 'list',
    ...
    columns: ['품목코드', '품명', '유형', '고객사', '고객품번', '차종', '규격', '재질', '단위(EA/KG)', '단중(KG)', '판매단가', '구매단가', '외주단가', '라우팅그룹', '대표위치', '사용'],
    feats: [
      ['이중단위·단중', 'EA/KG 이중단위와 단중(KG/EA)을 관리해 자재발주 총중량·수주 KG 환산에 사용'],
      ['고객(1차사) 품번', '사내 관리 품번(납품 품번 기준)과 별도로 고객 1차사 품번을 관리 — 도면·BOM 화면 연동 (개발 회의 반영)'],
      ['차종 자동연동', '품목에 차종을 입력하면 개발·도면 화면에서 자동 표시 (개발 화면 별도 입력 불필요)'],
    ] }),
  ...
  S({ id: 'drawing', title: '도면관리', group: '기준정보관리', path: '/base/drawings', type: 'masterDetail',
    purpose: '좌측 품목 리스트(자동 연동) + 우측 도면 4종(단품도·조립도·단조도·공구도) 단일 화면. 제품당 도면을 등록하고 개정 시 이력이 누적. PDF 업로드·POP 도면보기 지원 (개발 회의 3.3~3.6·3.8).',
    process: ['좌측 품목 선택', '도면 유형별 [등록] 또는 [개정]', '도면번호·Rev·도면명 입력', 'PDF 파일 드래그앤드롭/선택 또는 URL 입력', '저장(개정 시 이전본 이력화)'],
    relations: [
      { field: '품목 리스트', from: '품목관리', desc: '좌측에 자동 연동, 차종·고객품번 자동 표시' },
      { field: 'POP 도면보기', from: '작업 POP', desc: '[도면 보기] 버튼으로 작업자 화면에서 현행 도면 열람' },
      { field: '작성자', from: '사용자관리', desc: '도면 작성자 지정' },
    ],
    dropdowns: [{ field: '도면 유형', source: '고정값', values: '단품도/조립도/단조도/공구도' }, { field: '작성자', source: 'users', values: '등록 사용자' }],
    search: ['좌측 품목 검색: 품목코드·품명'], sort: '품목코드 · 유형 · 개정 내림차순',
    columns: ['(좌) 품목', '(우) 도면유형 4종 카드', '도면번호', 'Rev', '개정이력(누적)', '파일'],
    feats: [
      ['좌 품목 + 우 도면 단일화면', '품목 선택 시 우측에 단품도·조립도·단조도·공구도 4종을 카드로 표시, 현행 Rev·파일 링크 제공'],
      ['개정 이력 누적', '[개정] 시 새 Rev 등록·이전본은 이력(폐지)으로 보존, 하단 개정 이력 표에 전체 누적 표시'],
      ['PDF 업로드(드래그앤드롭)', '파일 선택 또는 드래그앤드롭으로 PDF 등록(최대 5MB), 또는 스토리지 URL 직접 입력'],
      ['차종·고객품번 자동표시', '품목의 차종·고객(1차사) 품번을 우측 상단에 자동 표시'],
      ['POP 도면보기', 'POP 작업자 화면 [도면 보기]에서 현행 도면을 새 탭으로 열람'],
    ],
    alerts: ['도면번호 미입력 시 저장 불가', '5MB 초과 파일은 URL 링크 사용 권장'],
    sq: ['현행 도면의 개정 관리', '작업자 도면 접근성', '도면-품목 연동'] }),
  ...
  S({ id: 'warehouse', title: '창고관리', group: '기준정보관리', path: '/base/warehouses', type: 'list',
    purpose: '계획생산 전환에 따라 자재·외주·중간공정(반제품)·완제품 창고를 기준정보로 신설. 입고 위치 이동과 반제품 재고 구분에 사용 (생산 회의 5.2).',
    relations: [{ field: '입고창고', from: '작업지시관리', desc: '완료품 입고 위치(반제품→중간공정창고, 완제품→완제품창고)' }, { field: '관리자', from: '사용자관리', desc: '창고 담당자 지정' }],
    dropdowns: [{ field: '창고유형', source: '고정값', values: '자재창고/외주창고/중간공정창고/완제품창고' }],
    search: ['검색: 창고코드·창고명·유형', '필터·칩: 창고유형'], sort: '창고코드 오름차순',
    columns: ['창고코드', '창고명', '유형', '위치', '관리자', '사용', '비고'],
    feats: [['창고 4종 구분', '자재·외주·중간공정(반제품)·완제품 창고를 구분 관리하여 반제품 재고와 완제품 재고를 분리']] }),
  ...
  S({ id: 'pop', title: '작업 POP', group: '작업 POP', path: '/pop', type: 'pop',
    purpose: '현장 키오스크 단말. 작업지시별 공정 시작/종료, 실적·자재·공구 투입 기록. 작업 시작 시 4M 조건을 스냅샷으로 보존하고, 생산 중 4M 변경 시 변경 전/후 LOT를 분리 관리.',
    process: ['작업자 선택', '작업지시 카드 선택(라우팅 전개)', '공정 시작(작업자·설비호기 → 4M 스냅샷 저장)', '자재·공구 투입', '(필요 시) 생산 중 4M 변경 등록', '공정 종료(생산·불량 입력)', '실적 자동등록 · 다음 공정 이동'],
    relations: [{ field: '공정 전개', from: '제품별표준공정(라우팅)', desc: '라우팅 순서대로(사내/외주)' }, { field: '설비호기 후보', from: '표준공정관리', desc: '공정 지정 설비로 제한' }, { field: '투입 자재', from: 'BOM관리', desc: '구성품' }, { field: '투입 공구 LOT', from: '공구 재고관리', desc: '공정 지정 공구 단위 LOT' }, { field: '4M 스냅샷', from: 'wo_4m_snapshots', desc: '작업 시작·변경 시점의 4M(작업자·설비·자재·작업표준서 버전) 세그먼트 보존' }, { field: '4M 변경', from: '4M 관리', desc: '생산 중 변경 등록·승인 연계(미승인 시 작업 재개 제한)' }, { field: '실적 생성', from: '생산실적', desc: '공정 종료 시 세그먼트 LOT로 자동 등록' }],
    dropdowns: [{ field: '작업자', source: 'users', values: '등록 사용자' }, { field: '설비호기', source: 'process_equipments', values: '공정 지정 설비' }, { field: '공구 LOT', source: '입고 단위 LOT', values: '잔여수명 있는 LOT' }, { field: '생산 중 4M 구분·등급', source: '버튼선택', values: 'Man/Machine/Material/Method · 단순이력/내부승인/중요4M/비상' }],
    search: ['작업자 선택', '상태 필터(대기/작업중/완료)'], sort: '작업지시일 순',
    columns: ['(카드) 작업지시·LOT·품명', '공정 진행(사내/외주)', '시작/종료 시각', '생산/불량/양품(자동)', '(패널) 생산 중 4M 변경 이력'],
    feats: [
      ['4M 스냅샷 저장', '공정 시작 시 작업자·설비·자재·작업표준서 버전을 최초 세그먼트(seg1)로 자동 저장'],
      ['생산 중 4M 변경', '[4M 변경] 버튼으로 변경 전 생산수량을 기록하고 변경 후를 신규 LOT 세그먼트로 분리(LOT-…-C1, -C2…)'],
      ['승인 대기 잠금', '내부승인·중요4M·비상 등급은 4M 관리 승인 전까지 [4M 승인대기]로 표시되어 작업 재개 제한'],
      ['4M 변경 이력 패널', '해당 작업지시의 생산 중 변경을 변경 전/후 LOT·수량·상태로 표시'],
      ['이상 발생(MRB)', '일시정지와 별도의 [이상 발생] 버튼으로 이상 유형(비가동 코드/품질 이상)·정지시간·상세를 기록하여 비가동 실적으로 연계 (생산 회의 반영)'],
      ['초품 검사 인터록', '작업 시작 시 대표 불량 확인 체크 필수 → 초품(시제품) 검사값이 검사규격 대비 전 항목 OK여야 [진행] 전환. NG면 세팅 상태로 잠겨 종료 불가 (생산 회의 1·2)'],
      ['초·중·종물 측정', '초품 OK 후 초물검사 진행, 중·종물 선택 측정. 검사규격(공정검사)의 기준값·상·하한으로 OK/NG 자동판정, 최종 입력값이 최종 측정값'],
      ['불량등록 버튼', '종료가 아닌 생산 중 [불량등록] 버튼으로 불량유형·수량을 등록 → 부적합관리에 미판정으로 표시. 종료 시 양품 = 투입 − 등록 불량 (생산 회의 3·4)'],
      ['작업자 변경', '작업자 교대는 4M 변경(Man·단순이력)으로 설비 고정·작업자 재선택·세그먼트별 시간 기록 (생산 회의 4.4)'],
    ] }),

  // ===== 영업관리 =====
  S({ id: 'sales-order', title: '수주관리', group: '영업관리', path: '/sales/orders', type: 'list',
    purpose: '고객 수주 등록. 생산 파이프라인 출발점. 고객 발주번호(PO)·KG 환산 관리. 긴급 발주 구분과 고객사≠납품처 분리 지원.',
    relations: [{ field: '거래처(고객사)', from: '거래처관리', desc: '매출처 드롭다운' }, { field: '납품처', from: '거래처관리', desc: '고객사와 다를 경우 별도 지정(분납 시 납품처 선택)' }, { field: '품명·규격·단가', from: '품목관리', desc: '품목 선택 시 자동' }, { field: '긴급·납품처 연계', from: '출하지시', desc: '수주 선택 시 긴급 표시·납품처 자동 연계' }, { field: '생산계획', from: '생산계획관리', desc: '계획 생성 시 상태 생산중' }],
    dropdowns: [{ field: '거래처(고객사)', source: 'partners', values: '등록 거래처' }, { field: '납품처', source: 'partners', values: '등록 거래처' }, { field: '긴급 발주', source: '스위치', values: 'ON/OFF' }, { field: '품목', source: 'items', values: '등록 품목' }, { field: '상태', source: '고정값', values: '접수/생산중/완료/취소' }],
    search: ['수주일 기간 조회', '필터·칩: 상태', '검색: 수주번호·거래처·품목'], sort: '수주일 내림차순',
    columns: ['긴급', '수주번호', '수주일', '거래처(고객사)', '납품처', '품목코드', '품명', '수주수량', '단가', '금액', '납기일', '상태'],
    feats: [
      ['긴급 발주 구분', '긴급 스위치로 표시하면 목록·출하지시까지 [긴급] 빨강 배지로 연계 표시 (영업 회의 반영)'],
      ['고객사≠납품처 분리', '고객사와 실제 납품처가 다른 경우 납품처를 별도 지정, 출하지시에 자동 연계 (분납 시 납품처 선택)'],
    ] }),
  ...
  S({ id: 'ship-order', title: '출하지시', group: '영업관리', path: '/sales/shipping-orders', type: 'list',
    purpose: '수주 기반 출하지시 등록. 출하검사·출하실적과 연계. 수주의 긴급 표시·납품처가 자동 연계.',
    relations: [{ field: '수주정보', from: '수주관리', desc: '수주 선택 시 거래처·납품처·긴급·품목·수량 자동' }, { field: '출하검사', from: '출하검사', desc: '지시 건 검사 연계' }],
    dropdowns: [{ field: '수주번호', source: 'sales_orders', values: '등록 수주(긴급 표시)' }, { field: '납품처', source: 'partners', values: '등록 거래처(수주 자동, 변경 가능)' }, { field: '출하창고', source: '고정값', values: '제품창고/자재창고1/2' }, { field: '상태', source: '고정값', values: '지시/출하완료/취소' }],
    search: ['출하예정일 기간 조회', '칩: 상태', '검색: 출하지시번호·수주번호·거래처'], sort: '출하예정일 내림차순',
    columns: ['긴급', '출하지시번호', '출하예정일', '수주번호', '거래처(고객사)', '납품처', '품명', '지시수량', '출하창고', '상태'],
    feats: [['긴급·납품처 연계', '수주에서 긴급 표시와 납품처를 자동 승계하여 출하 단계까지 [긴급] 배지·납품처를 표시']] }),
  ...
  S({ id: 'plan', title: '생산계획관리', group: '생산관리', path: '/production/plans', type: 'list',
    purpose: '수주를 생산계획으로 전개(엑셀 계획 대체), 일정·라인 배정. 계획 대비 진척 관리. 수주생산과 계획생산(발주 없이 반제품 등 직접 지시)을 함께 지원.',
    process: ['(수주생산) 상단 "생산계획 대기 수주"에서 [생산계획 생성]', '(계획생산) 신규등록 → 생산구분=계획생산 → 품목(반제품 포함) 직접 선택', '계획일·수량·기간 지정', '작업지시로 전개'],
    relations: [{ field: '수주번호·품목·수량', from: '수주관리', desc: '수주생산 시 수주 선택 자동' }, { field: '품목(반제품)', from: '품목관리', desc: '계획생산은 품목 직접 선택(반제품 SG 포함)' }, { field: '작업지시', from: '작업지시관리', desc: '계획→지시 전개' }],
    dropdowns: [{ field: '생산구분', source: '고정값', values: '수주생산/계획생산' }, { field: '수주', source: 'sales_orders', values: '등록 수주(수주생산 시)' }, { field: '품목', source: 'items', values: '등록 품목(계획생산 시)' }, { field: '상태', source: '고정값', values: '계획/진행/완료/보류' }],
    search: ['계획일 기간 조회', '필터·칩: 상태', '검색: 계획번호·수주·품목'], sort: '계획일 내림차순',
    columns: ['계획번호', '계획일', '생산구분', '수주번호', '품명', '계획수량', '시작일', '종료일', '상태'],
    feats: [['생산계획 대기 수주', '상단 패널에 계획 미수립 수주를 표시하고 즉시 생성'], ['계획생산 지원', '발주(수주) 없이 생산구분=계획생산으로 반제품 등 직접 생산계획 발행 (생산 회의 5.1)']] }),
  S({ id: 'wo', title: '작업지시관리', group: '생산관리', path: '/production/work-orders', type: 'list',
    ...
    columns: ['작업지시번호', 'LOT No.', '생산구분', '지시일', '품목코드', '품명', '지시수량', '입고창고', '생산계획', '계획시작', '계획종료', '상태'],
    feats: [['LOT No. 자동부여', '미입력 시 LOT-작업지시번호 형식으로 자동 부여(추적성 키)'], ['계획생산·입고창고', '생산구분=계획생산으로 발주 없이 지시, 완료품 입고창고(반제품→중간공정창고 / 완제품→완제품창고) 지정 (생산 회의 5.1·5.2)'], ['전표출력(바코드)', '[전표출력] → 작업지시서 겸 공정이동전표(Code39 바코드 + 라우팅 표) 인쇄'], ['작업시작', '[작업시작] 버튼 → 상태 작업중 → POP 표시']],
    alerts: ['전표 인쇄 시 팝업 차단 해제 필요', '계획기간(시작~종료)은 생산계획에서 자동 승계'] }),
  S({ id: 'time-adjust', title: '작업시간 수정(관리자)', group: '생산관리', path: '/production/time-adjust', type: 'list',
    purpose: '현장 POP에서는 시작·종료 시각을 수정할 수 없도록 하고, 관리자 전용 이 화면에서만 수정. 수정자·수정 사유가 함께 기록됨 (생산 회의 4.6).',
    relations: [{ field: '공정 시작/종료', from: '작업 POP', desc: '시작된 공정의 시각을 조회' }, { field: '수정자', from: '사용자관리', desc: '수정자 지정(기록)' }],
    dropdowns: [{ field: '수정자', source: 'users', values: '등록 사용자' }],
    search: ['검색: 작업지시·품목·공정·작업자'], sort: '시작시각 내림차순',
    columns: ['작업지시', '공정', '작업자', '설비', '시작', '종료', '작업(분)', '최근 수정', '관리'],
    feats: [['시각 수정·이력', '시작/종료 시각을 수정하면 작업시간 자동 재계산, 수정자·수정 사유·수정 시각을 기록']],
    alerts: ['수정자·수정 사유 미입력 시 저장 불가', 'POP 현장 화면에서는 시각 수정 불가'] }),
  ...
  S({ id: 'ncr', title: '부적합관리', group: '부적합관리', path: '/quality/nonconformance', type: 'masterDetail',
    ...
    feats: [
      ['POP 불량 자동 표시', 'POP [불량등록]으로 등록된 불량이 미판정으로 목록에 표시(작업지시·공정 연계)'],
      ['판정: 재작업/특채/폐기', '[판정] 버튼으로 관리자가 판정 — 특채: 완료수량 회복(양품+), 폐기: 차감 유지, 재작업: 재작업 작업지시 자동 생성(POP·작업리스트 [재작업] 표시) (생산 회의 5·6·7)'],
  ...
  S({ id: 'pfd', title: 'PFD (공정흐름도)', group: '개발관리', path: '/dev/pfd', type: 'masterDetail',
    ...
    feats: [
      ...
      ['특수공정 구분', '용접·열처리·도금 등 특수공정을 별도 체크하여 중점 관리 대상으로 구분'],
  ...
  S({ id: 'pfmea', title: 'PFMEA', group: '개발관리', path: '/dev/pfmea', type: 'masterDetail',
    ...
    feats: [
      ...
      ['AP·안전특성·과거불량', 'AIAG-VDA 조치우선순위(AP: Low/Medium/High), 안전특성 구분, 과거 고객/공정 불량 반영 이력을 항목별로 기록'],
  ...
  S({ id: 'cplan', title: '관리계획서', group: '개발관리', path: '/dev/control-plan', type: 'masterDetail',
    ...
    feats: [
      ...
      ['기록방법 지정', '검사일지·SPC 관리도·설비 데이터·체크시트·시스템 자동 중 기록방법을 항목별로 지정'],
  ...
  S({ id: 'wstd', title: '작업표준서', group: '개발관리', path: '/dev/work-standard', type: 'masterDetail',
    ...
    feats: [
      ...
      ['체결토크·용접조건', '체결토크(N·m)와 용접 전류·전압·시간 등 핵심 작업조건을 별도 항목으로 명시'],
  ...
  S({ id: 'fourm', title: '4M 관리', group: '변경관리', path: '/change/four-m', type: 'masterDetail',
    purpose: 'Man·Machine·Material·Method 변경을 변경등급별로 사전 승인하고, 생산 중 발생한 변경은 변경 전/후 LOT·생산이력을 분리 관리. 승인되지 않은 변경으로 인한 품질 변동을 방지.',
    process: ['4M 구분·변경등급 선택(버튼)', '변경 전 자동호출 → 변경 후 입력', '품질 위험성 검토·초도품·집중검사 입력', '관련 문서 개정 필요 여부 체크', '등급별 승인(고객승인·집중검사 검증)', '적용일·적용 LOT 기록'],
    relations: [
      { field: '변경 전 정보', from: '라우팅/BOM', desc: 'Machine·Material 선택 시 현재 정보 자동호출' },
      { field: '생산 중 변경', from: '작업 POP', desc: 'POP의 [4M 변경]으로 등록된 건이 생산연계(생산중변경) 경로로 표시' },
      { field: '4M 스냅샷', from: 'wo_4m_snapshots', desc: '변경 전/후 LOT 세그먼트·생산수량 분리 표시' },
      { field: '관련 문서', from: 'PFMEA·PFD·관리계획서·작업표준서', desc: '개정 필요 여부 체크 및 문서 상태 표시' },
      { field: 'PPAP', from: 'PPAP 승인관리', desc: '중요 4M 변경 시 재승인 대상 표시' },
    ],
    dropdowns: [{ field: '4M 구분', source: '버튼선택', values: 'Man/Machine/Material/Method' }, { field: '변경 등급', source: '버튼선택', values: '단순이력/내부승인/중요4M/비상' }, { field: '품목·공정', source: 'items/processes', values: '등록 데이터' }, { field: '승인상태', source: '고정값', values: '신청/검토중/승인/반려' }, { field: '적용상태', source: '고정값', values: '미적용/적용중/완료' }],
    search: ['필터: 4M 구분', '칩: 전체/신청/검토중/승인/반려/PPAP대상', '검색: 변경번호·품목·공정·사유'], sort: '변경일 내림차순',
    columns: ['변경번호', '변경일', '4M', '등급', '품명', '공정', '변경사유', '경로', '시험생산', '고객승인', '문서개정', '적용LOT', '승인상태', '적용'],
    feats: [
      ['변경 등급 관리', '단순이력·내부승인·중요4M·비상 4단계로 구분하여 승인 수준을 차등 적용(단순이력은 이력만, 중요4M은 고객승인, 비상은 집중검사 필수)'],
      ['생산 중 변경 이력 분리', 'POP에서 발생한 변경은 변경 전 생산분(기존 LOT)과 변경 후 생산분(신규 LOT)을 분리 집계하여 상세에 표시'],
      ['4M 버튼 선택·자동호출', 'Man·Machine·Material·Method 선택 시 변경 전/후 라벨과 라우팅·BOM 정보를 자동 입력'],
      ['품질 위험성 검토', '품질 위험성·초도품 검사결과·변경 후 집중검사·고객통보·격리 필요를 기록'],
      ['등급별 승인 검증', '중요4M은 고객승인, 비상은 집중검사, 내부승인 이상은 초도품 검증이 없으면 승인 시 경고'],
      ['문서 개정 연계', '체크한 문서의 실제 개정 상태(문서번호·Rev·승인)를 상세에서 확인'],
    ],
    alerts: ['중요4M·비상 변경은 고객승인/집중검사 없이 승인 시 경고', '생산 중 변경은 변경 전 정보를 수정하지 않고 신규 세그먼트로 분리', '승인 전 작업 재개 제한(POP 연계)'],
    sq: ['4M 변경의 등급별 사전승인', '승인되지 않은 임의 변경 방지', '생산 중 변경의 전/후 이력 추적', '초도품·집중검사 검증', '관련 문서 동시 개정'] }),
  S({ id: 'ppap', title: 'PPAP 승인관리', group: '변경관리', path: '/change/ppap', type: 'masterDetail',
    purpose: '양산부품 생산공정이 고객 요구사항을 만족함을 증명·승인받는 절차. AIAG 제출자료 13종 체크리스트로 미첨부 문서를 확인하고, 승인 이후 중요 4M 변경 발생 시 재승인 대상을 자동 표시.',
    process: ['PPAP 등록(고객사·품목·차종·Level·사유)', '제출자료 13종 체크(개발문서 자동체크)', '제출', '승인/조건부승인/반려', '반려 시 보완 후 재제출(차수+1)', '중요 4M 변경 발생 시 재승인'],
    relations: [
      { field: '개발문서', from: 'PFD·PFMEA·관리계획서·작업표준서', desc: '[자동 체크]로 승인 문서 확인' },
      { field: '연계 4M / 재승인', from: '4M 관리', desc: '승인 이후 중요4M·비상 변경 발생 시 재승인 필요 배너 표시' },
    ...
    columns: ['PPAP번호', '고객사', '품목', '품번', 'Level', '제출차수', '제출일', '서류(n/13)', '제출사유', '4M변경', '승인일', '적용일', '상태'],
    feats: [
      ['제출자료 13종 체크리스트', 'PFD·PFMEA·관리계획서·작업표준서·도면·치수측정·재질/성능·공정능력(Cpk)·MSA·외관한도견본·초도품·PSW·고객승인문서를 체크'],
      ['개발문서 자동체크', '품목의 승인된 개발문서를 조회해 자동으로 체크'],
      ['재승인 자동감지', '승인 이후 해당 품목에 중요4M·비상 변경이 발생하면 [재승인 필요] 배너와 변경 목록을 표시'],
      ['재제출', '[재제출] 버튼으로 제출차수를 자동 증가시키고 상태 초기화'],
      ['보완요청 관리', '조건부승인·반려 시 보완요청 내용과 기한을 기록'],
    ],
    alerts: ['서류 미첨부 상태로 승인 시 확인 경고', '미승인 품목의 양산 적용 주의', '중요 4M 변경 후 재승인 미실시 시 배너 경고'],
    sq: ['승인 조건과 실제 생산조건 일치', '미승인 품목 양산·납품 방지', '제출자료 완결성(AIAG 13종)', '설계·4M 변경 후 재승인', '승인문서와 양산 적용일 연결'] }),
