...
const SYSTEM_FLOWS = [
  ...
  { label: '개발 → 변경관리 흐름', tone: 'brand', steps: ['PFD (공정흐름도)', 'PFMEA', '관리계획서', '작업표준서', '4M 관리', 'PPAP 승인관리'] },
...
const SCREENS = [
  ...
  S({ id: 'pfmea', title: 'PFMEA', group: '개발관리', path: '/dev/pfmea', type: 'masterDetail',
    purpose: '공정별 잠재적 고장형태·영향·원인을 분석하고 S·O·D로 위험도(RPN)를 산출. 고위험 항목의 개선 전·후를 비교.',
    process: ['문서 등록(품목·개정)', 'PFD 공정 불러오기', '고장형태·영향·원인·관리 입력', 'S·O·D 입력 → RPN 자동계산', '개선대책 수립 → 개선 후 RPN 비교', '승인'],
    relations: [
      { field: '공정순서', from: 'PFD', desc: '[PFD 공정 불러오기]로 자동 호출' },
      { field: '품목·고객사', from: '품목관리', desc: '선택 시 고객사·제목 자동' },
      { field: '관리계획서 연계', from: '관리계획서', desc: '특별특성 항목이 관리항목으로 전개' },
      { field: '4M 변경', from: '4M 관리', desc: '변경 시 개정 필요 여부 체크' },
    ],
    dropdowns: [{ field: '공정', source: 'processes', values: '등록 공정' }, { field: '특성', source: '고정값', values: '일반/중요특성/특별특성' }, { field: '상태', source: '고정값', values: '작성중/검토중/승인/개정/폐기' }],
    search: ['검색: 문서번호·품목·제목', '필터: 품목·상태'], sort: '품목 → 개정 내림차순',
    columns: ['문서번호', '고객사', '품목', '품명', '개정', '제목', '작성자', '작성일', '승인자', '상태'],
    feats: [
      ['PFMEA 분석표', '공정·기능·고장형태·영향·S·원인·O·예방관리·검출관리·D·RPN·특성·개선대책·개선 후 RPN을 표에서 직접 입력'],
      ['RPN 자동계산', 'S×O×D를 실시간 계산. 100 이상 빨강, 60 이상 주황으로 강조'],
      ['PFD 공정 불러오기', '동일 품목 PFD의 공정순서를 자동 호출'],
      ['개정·복사', '[개정]으로 새 Rev 생성(항목 복사), 유사품목 복사 지원'],
      ['인쇄', '결재란 포함 PFMEA 양식(A4 가로) 인쇄'],
    ],
    alerts: ['상세 항목이 없는 문서는 승인 불가', '승인 시 이전 개정본은 개정 상태로 자동 변경', '승인된 문서는 삭제 불가'],
    sq: ['공정별 잠재불량 분석', '고위험 항목 개선', 'PFMEA와 관리계획서 연계', '최신 개정 및 승인관리'] }),
  S({ id: 'pfd', title: 'PFD (공정흐름도)', group: '개발관리', path: '/dev/pfd', type: 'masterDetail',
    purpose: '품목별 공정 순서와 투입품·산출품·검사공정을 정의. PFMEA·관리계획서의 공정 기준.',
    process: ['문서 등록', '표준 라우팅 불러오기', '공정 순서·구분·설비·투입/산출품 입력', '검사공정 표시', '승인'],
    relations: [{ field: '표준 라우팅', from: '제품별표준공정(라우팅)', desc: '[표준 라우팅 불러오기]로 자동 호출' }, { field: '설비', from: '설비관리', desc: '공정별 설비 선택' }, { field: 'PFMEA·관리계획서', from: '개발관리', desc: '공정순서를 두 문서에서 호출' }],
    dropdowns: [{ field: '공정코드', source: 'processes', values: '등록 공정' }, { field: '공정구분', source: '고정값', values: '가공/검사/이동/보관/외주/조립/포장' }, { field: '설비', source: 'equipments', values: '등록 설비' }],
    search: ['검색: 문서번호·품목·제목', '필터: 품목·상태'], sort: '품목 → 개정 내림차순',
    columns: ['문서번호', '고객사', '품목', '품명', '개정', '제목', '작성자', '작성일', '상태'],
    feats: [['공정흐름 표', '순번·공정코드·공정명·구분·설비·투입품·산출품·검사공정 여부를 표에서 입력'], ['순서 변경', '▲ 버튼으로 공정 순서를 변경(순번 자동 재부여)'], ['표준 라우팅 불러오기', '기준정보의 라우팅(사내/외주 구분 포함)을 그대로 호출']],
    alerts: ['상세 항목이 없는 문서는 승인 불가'],
    sq: ['실제 생산공정과 PFD 일치', '공정순서 및 검사공정 표시', 'PFMEA 및 관리계획서와 일관성', '개정이력 관리'] }),
  S({ id: 'cplan', title: '관리계획서', group: '개발관리', path: '/dev/control-plan', type: 'masterDetail',
    purpose: '공정별 관리항목·규격·관리방법·검사주기·계측기·이상 시 조치를 정의. PFD·PFMEA·검사규격과 연계.',
    process: ['문서 등록', 'PFD·PFMEA 항목 불러오기', '관리항목·규격·관리방법·주기·계측기 입력', '이상 시 조치 지정', '승인 → 현장 적용'],
    relations: [
      { field: 'PFD 공정', from: 'PFD', desc: '[PFD·PFMEA 불러오기]로 공정 자동 호출' },
      { field: 'PFMEA 특별특성', from: 'PFMEA', desc: '특별·중요특성 항목이 관리항목으로 자동 전개' },
      { field: '계측기', from: '계측기관리', desc: '관리항목별 계측기 지정' },
      { field: '검사규격', from: '검사규격관리', desc: '규격·주기 일치 필요' },
      { field: '작업표준서', from: '작업표준서', desc: '관리항목을 품질확인사항으로 전개' },
    ],
    dropdowns: [{ field: '공정', source: 'processes', values: '등록 공정' }, { field: '특성', source: '고정값', values: '일반/중요특성/특별특성' }, { field: '검사주기', source: '고정값', values: '초물/중물/종물/1회LOT/전수/주기' }, { field: '계측기', source: 'measuring_instruments', values: '등록 계측기' }, { field: '이상 시 조치', source: '고정값', values: '설비정지 후 보고/전수선별/재작업/공구교체/부적합 등록/초물 재검사' }],
    search: ['검색: 문서번호·품목·제목', '필터: 품목·상태'], sort: '품목 → 개정 내림차순',
    columns: ['문서번호', '고객사', '품목', '품명', '개정', '제목', '작성자', '작성일', '상태'],
    feats: [['관리항목 표', '공정·관리항목·특성·규격·관리방법·검사주기·샘플수·계측기·이상 시 조치를 표에서 입력'], ['PFD·PFMEA 불러오기', '공정순서와 특별특성 항목을 한 번에 호출'], ['인쇄', '관리계획서 양식(A4 가로) 인쇄']],
    alerts: ['상세 항목이 없는 문서는 승인 불가', '특별특성 항목은 관리방법·주기·계측기 지정 필요'],
    sq: ['PFD·PFMEA·관리계획서 연계', '특별특성 관리', '검사방법·주기·계측기 지정', '이상 발생 시 조치기준'] }),
  S({ id: 'wstd', title: '작업표준서', group: '개발관리', path: '/dev/work-standard', type: 'masterDetail',
    purpose: '공정별 작업단계·작업방법·작업조건·주의사항·품질확인사항을 정의. 승인된 최신본만 현장(POP)에 표시.',
    process: ['문서 등록(품목·공정)', '관리계획서 항목 불러오기', '작업단계별 방법·조건·사진 입력', '품질확인사항·이상 시 조치 입력', '승인 → 현장 표시'],
    relations: [{ field: '관리계획서', from: '관리계획서', desc: '[관리계획서 항목 불러오기]로 품질확인사항 자동 생성' }, { field: '공정', from: '표준공정관리', desc: '문서 대상 공정' }, { field: '현장 표시', from: '작업 POP', desc: '승인 최신본 표시' }],
    dropdowns: [{ field: '공정', source: 'processes', values: '등록 공정' }, { field: '상태', source: '고정값', values: '작성중/검토중/승인/개정/폐기' }],
    search: ['검색: 문서번호·품목·제목', '필터: 품목·상태'], sort: '품목 → 개정 내림차순',
    columns: ['문서번호', '고객사', '품목', '품명', '공정', '개정', '제목', '작성자', '작성일', '상태'],
    feats: [['작업단계 표', '단계명·작업방법·작업조건·사용공구·품질확인사항·주의사항·이상 시 조치·사진 URL을 입력'], ['관리계획서 연계', '해당 공정 관리항목을 품질확인사항으로 자동 전개'], ['인쇄', '작업표준서 양식 인쇄']],
    alerts: ['상세 항목이 없는 문서는 승인 불가', '승인본만 현장에 표시'],
    sq: ['표준작업 방법', '작업조건 및 품질 주의사항', '최신본 현장 적용', '작업표준과 관리계획서 일치'] }),
